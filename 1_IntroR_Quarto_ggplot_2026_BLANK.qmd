...
THIS TUTORIAL COMPLETED BY: Ren Miner
...
#glimpse gives summary of rows and columns
```

```{r}
View(candy_rankings)
#view gives you the data in spreadsheet form 
```
...
```{r}
ggplot(candy_rankings) + aes(x=chocolate) + geom_bar() # this says what kind of plot to make, in this case, a barplot that automatically did counts of the observations of which candies were chocolate or not chocolate. 
```
...
```{r}
chocolate_bar <- ggplot(candy_rankings) + aes(x=chocolate) + geom_bar()
...
sweetness #don't forget to call the plot after you create in in above code
```
...
```{r}
sweetness + 
  geom_smooth(method="lm") #lm means linear model, ie giving it a line 
```
...
sweetness <- ggplot(candy_rankings) +
  ...
  geom_boxplot()
sweetness
#by calling this plot sweetness, the one we created above was written over and lost
```
...
```{r}
ggplot
fruityprice<-ggplot(candy_rankings)+
...
2.  Are sweeter candies more expensive? Do the candies with higher sugarpercent have higher winpercent? Plot this relationship.
    ...
    ```{r}
    ggplot() #showing if sweeter candies are more expensive
    sweetprice<-ggplot(candy_rankings)+
      aes(x=sugarpercent,y=pricepercent)+
      geom_point()
    sweetprice +
...
```{r}
ggplot() #sugar percent vs win percent 
sweetwin<-ggplot(candy_rankings)+
